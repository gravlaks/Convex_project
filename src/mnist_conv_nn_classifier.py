...
if __name__ == '__main__':
    ...
    X_est,train_errors_05, _ = optimize(nn_gn, X0, train_X, train_y, steps=1000, batch_size=400, max_time=MAX_TIME, backtrack=True,
                        optimization_method="Random columns", optim_params={"keep_prob":0.5})
    print("Train Accuracy", get_accuracy(nn_gn, train_X, train_y))
    print("Test Accuracy", get_accuracy(nn_gn, test_X, test_y))
    X_est,train_errors_01, _ = optimize(nn_gn, X0, train_X, train_y, steps=1000, batch_size=400, max_time=MAX_TIME, backtrack=True,
                        optimization_method="Random columns", optim_params={"keep_prob":0.1})
    print("Train Accuracy", get_accuracy(nn_gn, train_X, train_y))
    print("Test Accuracy", get_accuracy(nn_gn, test_X, test_y))
    X_est,train_errors_001, _ = optimize(nn_gn, X0, train_X, train_y, steps=1000, batch_size=400, max_time=MAX_TIME, backtrack=True,
                        optimization_method="Random columns", optim_params={"keep_prob":0.01})
    print("Train Accuracy", get_accuracy(nn_gn, train_X, train_y))
    print("Test Accuracy", get_accuracy(nn_gn, test_X, test_y))

    sgd_losses_lr05 = stochastic_gradient_descent(train_X, train_y, epochs=3000, nn_gn=nn_gn, max_time=MAX_TIME, batch_size=400, lr=0.5)
    print("Train Accuracy", get_accuracy(nn_gn, train_X, train_y))
    print("Test Accuracy", get_accuracy(nn_gn, test_X, test_y))
    
    sgd_losses_lr01 = stochastic_gradient_descent(train_X, train_y, epochs=3000, nn_gn=nn_gn, max_time=MAX_TIME, batch_size=400, lr=0.1)
    print("Train Accuracy", get_accuracy(nn_gn, train_X, train_y))
    print("Test Accuracy", get_accuracy(nn_gn, test_X, test_y))
    sgd_losses_lr001 = stochastic_gradient_descent(train_X, train_y, epochs=3000, nn_gn=nn_gn, max_time=MAX_TIME, batch_size=400, lr=0.01)
    print("Train Accuracy", get_accuracy(nn_gn, train_X, train_y))
    print("Test Accuracy", get_accuracy(nn_gn, test_X, test_y))
    sgd_losses_lr0001 = stochastic_gradient_descent(train_X, train_y, epochs=3000, nn_gn=nn_gn, max_time=MAX_TIME, batch_size=400, lr=0.001)

    ## Print results
    losses = [train_errors_05, train_errors_01, train_errors_001, sgd_losses_lr05, sgd_losses_lr01, sgd_losses_lr001, sgd_losses_lr0001]
    labels = ["Algo 2: keep_prob 0.5", "Algo 2: keep_prob 0.1", "Algo2 : keep_prob 0.01", "SGD lr: 0.5","SGD lr: 0.1", "SGD lr: 0.01", "SGD lr: 0.001"]
    print("Train Accuracy", get_accuracy(nn_gn, train_X, train_y))
    print("Test Accuracy", get_accuracy(nn_gn, test_X, test_y))
